...
const ImageUpload = () => {
    ...
    const updateImage = async () => {
        ...
        try {
            const res = await axiosInstance.put(`/api/image/upload/${userId}`, { avatar: image }, { headers: { "Content-Type": "multipart/form-data", } })
            console.log(res.data)
            alert('Image uploaded.')
            navigate('/user/' + userId)
        } catch (error) {
    ...
    return (
        <>
            ...
            <button onClick={() => updateImage()} className="px-4 py-1.5 rounded-lg bg-blue-400 text-white">Uploadate</button>
        </>
    )
}
