...
interface PropType { id: string, name: string, avatar: string, url: string }
...
const RenderData = ({ id, name, avatar, url }: PropType) => {
    return <Link to={url} className='w-full flex items-center  gap-2 bg-white hover:bg-purple-200 shadow py-1 px-2 rounded-md' key={id}>
                        ...
                        : <div className='flex flex-col gap-2 bg-neutral-100 border border-neutral-200 shadow rounded-lg p-4'>

                            {
                                <div className='space-y-2'>
                                    ...
                                    {
                                        searchResult.groups && searchResult.groups.map(g => <RenderData id={g._id} name={g.name} avatar={g.avatar!} url={'/group/' + g._id} key={g._id} />)
                            ...
                            }
                            {
                                ...
                                <div className='space-y-2'>
                                    ...
                                    {searchResult.users.map(u => <RenderData id={u._id} name={u.name} avatar={u.avatar!} url={'/user/' + u._id} key={u._id} />)}
                                </div>
                            }
